...
static inline void 	set_surface(t_obj **objects, t_obj *object, t_ray *hit, int depth)
{
  float		fresneleffect;
  t_vec3f	tmp;
  t_vec3f	tmp2;
  ...
  vec3f_add2(&tmp2, &tmp);
  trace(&((t_ray){INFINITY, tmp2, hit->refldir, -1}), objects, depth + 1, &tmp);
...
static void	transparency(t_obj **objects, t_ray *hit, int depth)
{
  float		eta;
  float		cosi;
  float		k;
  ...
  trace(&((t_ray){INFINITY, tmp, refrdir}), objects, depth + 1, &hit->refraction);
}

void			effects(t_obj **objects, t_obj *object, t_ray *hit, int depth)
{
  t_vec3f	tmp;
