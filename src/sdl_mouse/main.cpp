...
typedef struct	s_texture {
  ...
  int		height;
  SDL_Texture	*texture;
  int		(*texture_load_from_file)(struct s_texture *t, const char *path, SDL_Renderer *gRenderer);
...
int		texture_load_from_file(t_texture *self, const char *path, SDL_Renderer *gRenderer)
{
...
t_texture *gButtonSpriteSheetTexture = texture_new();
...
void LButton::render()
{
  ...
  gButtonSpriteSheetTexture->texture_render( gButtonSpriteSheetTexture,  mPosition.x, mPosition.y, &gSpriteClips[ mCurrentSprite ], 0.0, NULL, SDL_FLIP_NONE , gRenderer) ;
}
...
bool loadMedia()
{
	...
	//Load sprites
	if( !gButtonSpriteSheetTexture->texture_load_from_file(gButtonSpriteSheetTexture, "button.png", gRenderer))
	{
...
{
	//Free loaded images
	gButtonSpriteSheetTexture->texture_free(gButtonSpriteSheetTexture);
