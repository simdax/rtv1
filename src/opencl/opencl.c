	...
	t_cl *gpu, ...)
{
	...
	if (!init_program(&prog, s_buffers, n_buffers))
		return (NULL);
	i = -1;
	va_start(ap, *gpu);
	while (++i < n_buffers)
	{
		s_buffers.type[i] = va_arg(ap, t_type_buf);
		s_buffers.size[i] = va_arg(ap, size_t);
		s_buffers.name[i] = va_arg(ap, void*);
	}
	va_end(ap);
	create_buffers(&prog, s_buffers, n_buffers);
	i = -1;
	while (++i < n_buffers)
